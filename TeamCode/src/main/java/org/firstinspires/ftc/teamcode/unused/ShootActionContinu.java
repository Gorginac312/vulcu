...
public class ShootActionContinu {
    ...
    public void fireShots(double rpm, int numShots) {
        ...
        // ===== NEW START BEHAVIOR FOR 3 SHOTS =====
        ...
        if (numShots == 3) {
            ...
            // Move right now (this is the key fix)
            intake.Indexer.setPosition(values.OuttakePose1);

            timer.reset();
            stepStart = timer.milliseconds();

            // After the delay, go straight to WAIT_INDEXER (pose already commanded)
        ...
        switch (shootStep) {
            ...
            case PRE_DELAY:
                if (elapsedMs(currentPreDelayMs)) {
                    // Pose1 was already commanded in fireShots()
                    stepStart = timer.milliseconds();
    ...
    private double computeDelayToOuttakePose1Ms(double pos) {
        ...
        // If we're close to a known pose, use that directly
        ...
        // Otherwise choose the nearest of the 6 and use its delay
